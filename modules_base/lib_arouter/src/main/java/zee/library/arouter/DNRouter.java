...
import zee.library.arouter.exception.NoRouterFoundException;
import zee.library.arouter.template.IRouterGroup;
import zee.library.arouter.template.IRouterRoot;
import zee.library.arouter.template.IService;
import zee.library.arouter.utils.ClassUtils;
import zee.library.router_annotation.model.RouterMeta;

public class DNRouter {

    private static final String TAG = "DNRouter";
    private static final String ROUTE_ROOT_PACKAGE = "zee.library.arouter";
    ...
    private static void loadInfo() throws InterruptedException, IOException, PackageManager
        ...
        for (String className : routerMap) {
            if (className.startsWith(ROUTE_ROOT_PACKAGE + "." + SDK_NAME + SEPARATOR +
                ...
                ((IRouterRoot) (Class.forName(className).getConstructor().newInstance())).loadInfo(Warehouse.groupsIndex);
            }
        }
    }
     ...
     * @param callback
     */
    protected Object navigation(Context context, final Postcard postcard,
    ...
    private void prepareCard(Postcard card) {
        ...
        // 还没准备的
        if (null == routerMeta) {
            // 创建并调用 loadInfo 函数,然后记录在仓库
            Class<? extends IRouterGroup> groupMeta = Warehouse.groupsIndex.get(card.getGroup());
            if (null == groupMeta) {
                throw new NoRouterFoundException("没找到对应路由: " + card.getGroup() + " " + card.getPath());
            }
            ...
            iGroupInstance.loadInfo(Warehouse.routers);
