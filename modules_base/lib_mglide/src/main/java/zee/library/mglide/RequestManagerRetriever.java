...
import zee.library.mglide.manager.ApplicationLifecycle;
import zee.library.mglide.manager.SupportRequestManagerFragment;
    ...
    private SupportRequestManagerFragment getSupportRequestManagerFragment(FragmentManager fm) {
        ...
        if (null == fragment) {
            ...
            if (null == fragment) {
                ...
                // Glide.with
                // Glide.with
                // 连续调用两次：第一次创建Frgament但是由于FM.add不是commitAllowingStateLoss就绑定上去了
